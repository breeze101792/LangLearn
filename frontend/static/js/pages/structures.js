...
export function renderStructures(host) {
  ...
  renderListPage({
    ...
    fillEndpoint: "/api/structures/fill",
...
async function editRow(items, btn, endpoint, fields, load) {
  ...
  if (!item) return;
  const newPattern = prompt("Pattern", item.pattern || "");
  if (newPattern == null) return;
  const newExample = prompt("Example sentence", item.example_sentence || "");
...
async function deleteRow(btn, items, endpoint, load) {
  const id = Number(btn.closest(".list-item").dataset.id || "0");
